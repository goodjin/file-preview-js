...
describe('planSendMessage', () => {
  test('adds system message once and returns assistant placeholder only for UI', () => {
    const p1 = planSendMessage([], 'hi', '你是一个有帮助的助手。');
    ...
    const p2 = planSendMessage(p1.messagesForUi, 'next', '你是一个有帮助的助手。');
  ...
  test('replaces system prompt content on next send', () => {
    const p1 = planSendMessage([], 'hi', 'system A');
    const p2 = planSendMessage(p1.messagesForUi, 'next', 'system B');
    expect(p2.messagesForModel[0]?.role).toBe('system');
    expect(p2.messagesForModel[0]?.content).toBe('system B');
  });
});
