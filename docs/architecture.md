...
Agent Society 提供了一个仿微信风格的 Web 界面，用于可视化展示智能体对话和组织结构。
...
| 组件 | 文件 | 职责 |
|------|------|------|
| App | `web/js/app.js` | 全局状态管理、路由、定时轮询 |
| AgentList | `web/js/components/agent-list.js` | 智能体列表渲染、筛选、排序 |
| ChatPanel | `web/js/components/chat-panel.js` | 对话消息显示、发送消息 |
| OverviewPanel | `web/js/components/overview.js` | 组织树状图、岗位统计 |
| MessageModal | `web/js/components/message-modal.js` | 消息详情弹窗 |
| Toast | `web/js/components/toast.js` | 通知提示 |

### 工具函数

| 模块 | 文件 | 功能 |
|------|------|------|
| SortUtils | `web/js/utils/sort.js` | 智能体列表排序 |
| FilterUtils | `web/js/utils/filter.js` | 智能体列表筛选 |
| TreeUtils | `web/js/utils/tree.js` | 组织树构建、岗位统计 |
| API | `web/js/api.js` | HTTP API 调用封装 |

### 消息持久化

消息使用 JSON Lines 格式持久化，按智能体 ID 分割文件：
...
每条消息同时写入发送者和接收者的文件（如果不同），确保查询任一智能体时都能获取完整对话。

### 数据流

1. **初始化**：页面加载时并行请求智能体列表、岗位列表、组织树
2. **选择智能体**：请求该智能体的消息列表并渲染
3. **轮询更新**：每 2 秒检查新智能体和新消息
4. **发送消息**：POST 到 `/api/send`，以 user 身份发送
